...
def binary_search(needle, haystack, start=0, length=None):
    if length is None:
        length = len(haystack) - 1

    if length >= 0:
        middle = start + (length - start) // 2  # Obtenemos la llave del valor medio
        ...
        if haystack[middle] == needle:  # Si el valor es el de la mitad
            ...
            # mitad menos 1
            return binary_search(needle, haystack, start, middle - 1)
